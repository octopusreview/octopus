 ...
 *
 * Cache lives in SystemConfig.latestRelease and is refreshed daily by the
 * "refresh-release-cache" pg-boss worker. On a cache miss (the daily job
 * hasn't run yet) we fetch synchronously through `getCachedOrFreshRelease`
 * and write through — the actual fetch + cache logic lives in
 * `apps/web/lib/releases.ts` so the worker can call it directly without
 * importing this route module (which would pull Next.js runtime into
 * the worker context).
 */
