...
async function gatherCrossFileContext(
  queries: { findingIndex: number; query: string; filePath?: string }[],
  repoId: string,
  orgId: string,
): Promise<string> {
...
Output ONLY the JSON array, nothing else.`,
  ...
  try {
    ...
    console.log(`[review-core] Two-pass validation: ${validated.length}/${findings.length} findings kept (threshold: ${confidenceThreshold})`);
    return validated;
  } catch {
